"""EduASM M2 assembler for EduCPU ISA v0."""
...
OPS={
"NOP":(0x00,[]),"HALT":(0x01,[]),
"MOV":(0x10,["r","r"]),"MOVI":(0x11,["r","i8"]),
"LOAD":(0x12,["r","a16"]),"STORE":(0x13,["a16","r"]),
"LOADR":(0x14,["r","r"]),"STORER":(0x15,["r","r"]),
"ADD":(0x20,["r","r"]),"ADDI":(0x21,["r","i8"]),
"SUB":(0x22,["r","r"]),"SUBI":(0x23,["r","i8"]),
"CMP":(0x24,["r","r"]),"CMPI":(0x25,["r","i8"]),
"AND":(0x28,["r","r"]),"OR":(0x29,["r","r"]),"XOR":(0x2A,["r","r"]),
"NOT":(0x2B,["r"]),"SHL":(0x2C,["r"]),"SHR":(0x2D,["r"]),
"JMP":(0x30,["a16"]),"JZ":(0x31,["a16"]),"JNZ":(0x32,["a16"]),
"JC":(0x33,["a16"]),"JNC":(0x34,["a16"]),"JN":(0x35,["a16"]),"JP":(0x36,["a16"]),
...
def parse(lines):
    ...
    for no,raw in enumerate(lines,1):
        ...
        rows.append((no,pc,name,args,raw.rstrip()))
        pc+=1+sum(1 if t in ("r","i8") else 2 for t in spec)
    return labels,rows
def assemble_text(text, listing=False):
    ...
    for no,pc,name,args,source in rows:
        ...
        out.extend(b); listing_rows.append((pc,bytes(b),source))
    return bytes(out),labels,listing_rows
def main():
    p=argparse.ArgumentParser(prog="eduasm");p.add_argument("source");p.add_argument("-o","--output");p.add_argument("-l","--listing",action="store_true");a=p.parse_args()
    src=Path(a.source); data,_,rows=assemble_text(src.read_text())
    dest=Path(a.output or src.with_suffix(".bin"));dest.write_bytes(data)
    if a.listing:
        for pc,b,s in rows:print(f"{pc:04X}  {b.hex(' ').upper():<14} {s}")
