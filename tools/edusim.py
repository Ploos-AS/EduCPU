...
class Simulator:
    ...
    def step(self):
        ...
        pc=c.pc;ins=instruction_at(c,pc);before=state(c);watched={a:c.mem[a] for a in self.watch};c.step();after=state(c);self.last=(pc,ins,before,after)
        print(f"{pc:04X}: {ins}");ch=diff(before,after)
        if ch:print("  "+", ".join(ch))
        for a,old in watched.items():
            if c.mem[a]!=old:print(f"  MEM[{a:04X}]: {old:02X}->{c.mem[a]:02X}")
    def microstep(self):
        before=state(self.cpu);step,committed=self.microstepper.next()
        if step is None:self.show();return
        print(f"{step.phase:<12} {step.text}")
        if committed:
            after=state(self.cpu);ch=diff(before,after)
            print("COMMIT       architectural instruction completed")
            if ch:print("  "+", ".join(ch))
    def micro(self):
        for s in plan(self.cpu):print(f"{s.phase:<12} {s.text}")
        print("Use 'microstep'/'ms' to advance this sequence one conceptual phase at a time.")
        print("Pedagogical phases are not a frozen hardware microarchitecture.")
...
def repl(sim):
    ...
    while True:
        ...
        try:
            ...
            elif cmd=="help":print("step|s, microstep|ms, micro, run|r [limit], state|regs, mem <addr> [count], break <addr>, watch <addr>, reset, quit")
            else:print("unknown command; type help")
        except (ValueError,IndexError) as e:print(f"error: {e}")
def main():
    p=argparse.ArgumentParser(prog="edusim");p.add_argument("binary");a=p.parse_args();repl(Simulator(Path(a.binary).read_bytes()))
