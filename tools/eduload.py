...
def frame(image: bytes) -> bytes:
    ...
    if len(image)>0xffff: raise ValueError("image exceeds protocol-v0 65535-byte limit")
    n=len(image); return bytes((0x55,0xaa,n&0xff,(n>>8)&0xff))+image

def image_bytes(path: pathlib.Path) -> bytes:
    suffix=path.suffix.lower()
    if suffix==".eduasm":
        from eduasm import assemble_text
        data,_,_=assemble_text(path.read_text()); return data
    if suffix==".educ":
        from educ import compile_source
        *_,data,_=compile_source(path.read_text(),str(path)); return data
...
def main():
    ...
    args=ap.parse_args(); data=image_bytes(args.image)
    if args.protocol=="v1":
        from eduload_protocol import frame_v1
        packet=frame_v1(data)
    else: packet=frame(data)
    try: import serial
    except ImportError: sys.exit("pyserial is required: python -m pip install pyserial")
    with serial.Serial(args.port,args.baud,bytesize=8,parity="N",stopbits=1,timeout=1) as s:
        s.reset_input_buffer(); s.write(packet); s.flush()
        if args.protocol=="v1":
            status=s.read(1); run=s.read(1)
            try: result=interpret_v1_status(status,run)
            except RuntimeError as e: sys.exit(str(e))
            print("loader ACK")
            if result=="HALT": print("CPU HALT")
