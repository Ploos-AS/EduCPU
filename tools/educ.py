...
def ast_dict(node):
 ...
 return node
def main():
 p=argparse.ArgumentParser(prog="educ");p.add_argument("source");p.add_argument("--tokens",action="store_true");p.add_argument("--ast",action="store_true");a=p.parse_args();text=Path(a.source).read_text()
 if a.tokens:
