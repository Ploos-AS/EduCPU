...
module tb_serial_v1_e2e;
 ...
 initial begin pending=0;mr=0;mrd=0;for(i=0;i<65536;i=i+1)mem[i]=0;repeat(4)@(posedge clk);reset=0;
  fork begin recv(a);recv(b);end begin send(8'h55);send(8'haa);send(8'h01);send(8'h04);send(0);send(8'h11);send(0);send(8'h2a);send(1);send(8'h8b);send(8'hc8);end end join
