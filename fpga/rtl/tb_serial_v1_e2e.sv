...
module tb_serial_v1_e2e;
 ...
 task send(input[7:0]b);integer k;begin @(negedge clk);rx=0;repeat(CPB)@(negedge clk);for(k=0;k<8;k=k+1)begin rx=b[k];repeat(CPB)@(negedge clk);end rx=1;repeat(CPB+2)@(negedge clk);end endtask
