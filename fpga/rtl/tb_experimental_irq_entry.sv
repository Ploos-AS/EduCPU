module tb_experimental_irq_entry;
 ...
 initial begin
  ...
  wait(state_write === 1'b1); #1;
  if(next_pc!=16'h8000||next_sp!=16'hfefd) $fatal(1,"entry state");
  if(mem[16'hfeff]!=8'h12||mem[16'hfefe]!=8'h34||mem[16'hfefd]!=8'h05) begin
   $display("stack %02x %02x %02x",mem[16'hfeff],mem[16'hfefe],mem[16'hfefd]);
   $fatal(1,"entry stack");
  end
  cpu_sp=16'hfefd;iret_request=1;tick;iret_request=0;
  wait(iret_complete === 1'b1); #1;
